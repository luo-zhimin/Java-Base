...
select * from emp where date(hiredate)> date('1992.1.1');
--  ■如何使用like操作符
--      %∶表示0到多个字符 _∶表示单个字符
--      ?如何显示首字符为S的员工姓名和工资
...
--  ?按照 部门号升序 而雇员的工资降序排列，显示雇员信息  order by xx 默认asc
select * from emp order by deptno,sal desc ;

# 分页
-- 推导一个公式
# SELECT * FROM xx LIMIT 每页显示记录数 * (第几页-1) , 每页显示记录数
...
# 按雇员的 empno 号降序取出，每页显示5条记录。请分别显示第3页，第5页
select * from emp order by empno desc limit 0,5; -- 一页
select * from emp order by empno desc limit 10,5; -- 三页
select * from emp order by empno desc limit 20,5; -- 五页

# 使用分组函数和分组子句 组合使用 having group by limit..

# (1)显示每种岗位的雇员总数、平均工资。
select COUNT(*),job,format(avg(sal),2) as sal from emp group by job;
# (2)显示雇员总数，以及获得补助的雇员数。
select COUNT(*),count(comm) from emp;
# 扩展要求：统计没有获得补助的雇员数
select count(if(comm is null,1,null)) as comm from emp;
# (3)显示管理者的总人数。
select COUNT(*) from emp where job='MANAGER';
# 4.显示雇员工资的最大差额
select (max(sal)-min(sal)) as sal from emp;

# 统计各个部门的平均工资 并且是大于1000 并且按照平均工资从高到低排序 取出前两行记录
# 数据分组的总结
# 如果 select 语句同时包含有 group by，having，limit order by 那么他们的顺序是 group by，having，order by,limit

select avg(sal) as avgSal,format(avg(sal),2),deptno
from emp
group by deptno
having avgSal>1000
